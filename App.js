...
// Full path data for "All Paths" modal
...
const IconSort = ({ size = 16, color = C.maroon, asc = true }) => (
  <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
    {asc ? (
      ...
      <>
        <Path d="M3 18h18M6 12h12M9 6h6" stroke={color} strokeWidth="2" strokeLinecap="round" />
      </>
    )}
  </Svg>
);
...
const PathsModal = ({ visible, onClose }) => {
  ...
  const [sortAsc, setSortAsc]         = useState(false); // default desc (most frequent first)
    ...
    <Modal visible={visible} animationType="slide" transparent statusBarTranslucent>
      <View style={styles.modalOverlay}>
        <View style={styles.modalSheet}>
          ...
          {/* Close dropdown if open and tap outside */}
          {monthDropOpen && (
            <TouchableOpacity
          ...
          <FlatList
            ...
            renderItem={({ item, index }) => {
              // Rank relative to current filtered+sorted list
              const rank = index + 1;
...
export default function App() {
  ...
  // Dashboard shows only first 5 history items
      ...
      >
        <Animated.View style={{ opacity: contentOp, transform: [{ translateY: contentY }] }}>
          ...
          <View style={styles.sectionCard}>
            ...
            {/* Row 2: subtitle (left) + Top 6 badge + View All button (right) */}
              ...
              {HISTORY_PREVIEW.map((item, index) => {
                ...
                return (
                  <View
                    ...
                    style={[
                      styles.historyItem,
                      index === HISTORY_PREVIEW.length - 1 && { borderBottomWidth: 0 },
                    ]}
                  >
                    <View style={[styles.historyBadge, { backgroundColor: t.bg }]}>
                      <Text style={[styles.historyBadgeText, { color: t.color }]}>{t.label}</Text>
